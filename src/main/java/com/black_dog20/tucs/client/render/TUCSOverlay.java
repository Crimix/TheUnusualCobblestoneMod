	...
	public void renderItem(ItemRenderType type, ItemStack itemStack,
		...
		if (icon != null) {
			...
			if (itemStack.getItem() instanceof ItemTLPOLM) {
				text = Integer.toString(InventoryHelper.findBlock(Blocks.torch,
						player.inventory));
				...
				if (itemStack.hasTagCompound()) {
					...
					if (!nbt.hasKey(NBTTags.NoArrow)
						...
						text = Integer.toString(InventoryHelper.findItem(
								Items.arrow, player.inventory));
					}
					if (nbt.hasKey(NBTTags.NoArrow)) {
				...
				text = Integer.toString(M1911Helper.getAmmo(itemStack))
						+ " / "
						+ Integer.toString(InventoryHelper.findItem(
								ModItems.ammo, player.inventory));
			} else if (itemStack.getItem() instanceof ItemTUCSUpgrades) {
				if (!itemStack.getDisplayName().replaceAll("[\\D]", "")
						.isEmpty()) {
					text = "LvL "
							+ itemStack.getDisplayName()
									.replaceAll("[\\D]", "");
				}
			} else if (itemStack.getItem() instanceof IScubaMask
