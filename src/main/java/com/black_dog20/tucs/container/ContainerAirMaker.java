...
public class ContainerAirMaker extends Container {
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int par1) {
		 ...
		 * (Slot)this.inventorySlots.get(par1);
		 * 
		 * if (slot != null && slot.getHasStack()) { ItemStack itemstack1 =
		 * slot.getStack(); itemstack = itemstack1.copy();
		 * 
		 * if (par1 == 2) { if (!this.mergeItemStack(itemstack1, 3, 39, true)) {
		 * return null; }
		 * 
		 * slot.onSlotChange(itemstack1, itemstack); } else if (par1 != 1 &&
		 * par1 != 0) { if (par1 >= 3 && par1 < 30) { if
		 * (!this.mergeItemStack(itemstack1, 30, 39, false)) { return null; } }
		 * else if (par1 >= 30 && par1 < 39 && !this.mergeItemStack(itemstack1,
